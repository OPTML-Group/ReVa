...
def run_rmu(
    ...
    # 加载拒答向量
    if hasattr(args, 'refusal_vector_path') and args.refusal_vector_path:
        print(f"加载拒答向量从: {args.refusal_vector_path}")
        refusal_vector = torch.load(args.refusal_vector_path, map_location=updated_model.device)
        print(f"拒答向量形状: {refusal_vector.shape}")
        print(f"拒答向量范数: {torch.norm(refusal_vector).item()}")
        
        control_vectors_list = []
        for i in range(len(forget_data_list)):
            # 使用拒答向量而不是随机向量
            control_vec = refusal_vector.to(dtype=updated_model.dtype, device=updated_model.device)
            # 应用steering coefficient
            control_vec = control_vec * args.steering_coeff_list[i]
            control_vectors_list.append(control_vec)
            print(f"主题 {i} 使用拒答向量，steering_coeff: {args.steering_coeff_list[i]}")
    else:
        # 如果没有提供拒答向量路径，使用原来的随机向量方法
        print("未提供拒答向量路径，使用随机向量")
        control_vectors_list = []
        for i in range(len(forget_data_list)):
...
def get_args():
    ...
    parser.add_argument("--refusal_vector_path", type=str, default=None, help="Path to the refusal vector file (.pt)")
